/**
 * Deploy the AgentBet Intelligent Contract to GenLayer StudioNet and write the
 * deployment artifact.
 *
 * Run:  npm run deploy        (needs OWNER_PK in .env)
 */
...
const SOURCE = path.join(ROOT, "contracts", "agentbet.py");

async function main() {
  const client = signer("OWNER_PK");
  const code = fs.readFileSync(SOURCE, "utf-8");

  console.log("AgentBet - deploying to StudioNet");
  ...
  const artifact = {
    ...
    deployTxHash: hash,
    contractFile: "contracts/agentbet.py",
  ...
  console.log(`  AGENTBET_ADDRESS=${address}`);
